...
```r
x = rnorm(500)
data = data.frame(x, x^2 + rnorm(500, sd = 0.2))
plot(data)
```
...
**Frage** Welche anderen Möglichkeiten gibt es denn noch mal außer metrisch
...
**Antwort** Eine Variable mit Werten: winzig, mittel, groß
...
**Antwort** Das ergibt sich direkt aus der Definition des p-Wertes - der p-Wert ist die Wahrscheinlichkeit des beobachteten Unterschieds (definiert durch die Teststatistik) oder größer, gegeben H0 - diese Definition impliziert, dass die Wahrscheinlihckeit  einen wert unter 5% zu bekommen 5% ist. 
...
**Frage** Geben Sie die false discovery rate an für das übliche Signifikanzlevel 0.05, Power = 0.4, p(H0) = 0.5

**Antwort** 0.05 * 0.5 / (0.05 * 0.5 + 0.4 * 0.5) -> Rate Typ I / (Rate Typ I + Rate echte Positive)


**Frage** Der p-Wert eines Experiments 0.04 - ist die folgende Aussage richtig: die Wahrscheinlichkeit dass H0 wahr ist is kleiner als 4%? Begründen Sie warum.

**Antwort** Nein. Begründung: p-Wert gibt Wahrscheinlichkeit der Daten gegeben H0 - wie wahrscheinlich es dann ist dass H0 oder nicht H0 zutrifft kann man daraus nicht sehen. Wenn überhaupt sagt dass die FDR. 
...
**Frage** Wie funktioniert die Methode der Maximum Likelihood Schätzung in Worten

**Antwort** Modell für Wahrscheinlichkeit der Daten gegeben die Parameter aufstellen, dann die Parameter suchen für die die Wahrscheinlichkeit der Daten maximal ist. 

Die folgenden Fragen beziehen sich auf diese Regression
...
**Frage** Schreiben sie die funktionale Form auf für die nach der Berechnung in R die Likelihood maximal ist (dem MLE)

**Antwort** roots = -33.54052 + 12.19976 * leaves + -0.09561 * leaves^2


**Frage** Der p-Wert für den Achsenabschnitt (Intercept) ist ja nicht signifikant. Aber testet eigentlich dieser Wert, d.h. was ist die Nullhypothese?
...
**Frage** Was ist denn überhaupt die Verteilung der Residuen?

**Antwort** Abstände zwischen der Modellvorhersage (Polynom, siehe Frage oben) und den Daten
...
**Antwort** Sie sollten normalverteilt sein. Die Verteilung sieht grob wie eine Normalverteilung aus (ein paar Schwankungen sind natürlich immer da)
...
**Frage** Nennen Sie mindestens 2 offensichtliche Störfaktoren um einen Zusammenhang zwischen Goldschmuck und Lebenserwartung nachzuweisen, zusammen mit der Erklärung warum diese Faktoren Störfaktoren sind (Korrelation mit erkl. und abh. Variable)
...
**Antwort** Internet Validität


**Frage** Erklären sie am Beispiel der oben genannten Studie das Konzept der externen Validität. 

**Antwort** externen Validität = kann man die Studienergebnisse verallgemeinern. Z.B. kann man die Ergebnisse auf die deutsche Bevölkerung oder Menschen im allgemeinen (ganze Welt) verallgemeinern
...
Wenn Sie Weihnachtsgrüße an einen befreundeten Statistiker schicken sollen, so versenden Sie doch einfach diese R Weihnachtskarte (natürlich als Code, ist ja klar), erstellt von der Kollegin [Carline Tucker](http://www.carolinemtucker.com/), zur Zeit CNRS in Montpellier. 
