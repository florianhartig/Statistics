...
\chapter{Einleitung} % Use chapters instead of sections
	...
	Kennzahlen der Statistik\marginnote{Kennzahlen der Statistik fassen die Daten zusammen} sind numerische Berechnungen, die Datens"atze zusammenfassen. Dies dient zur kompakten Veranschaulichung der Eigenschaften eines Datensatzes.
...
Wir benutzen das Mosaikplot. M"oglicherweise muss man zuvor das dazu benötigte Paket installieren: install.packages ("vcd")
