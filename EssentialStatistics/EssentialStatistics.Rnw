...
\begin{itemize}
\item Unordered (nominal) categorical variables (e.g. red, green, blue)
\item Binary (dichotomous) variables (e.g. dead / survived, 0/1)
\item Ordered (ordinal) categorical variables (e.g. small, medium, large)
\end{itemize}
\end{itemize}
...
How can we summarize the properties of this observed sample? Some basic properties would be the minimum and the maximum value, the mean, or the mode (the maximum of the distribution, i.e. value with the highest density of observations). There are two further central summaries that are much used: moments and quantile.

The moments may not sound familiar to you, but you probably have already used at least two of them: the mean and the standard deviation. In general, the n-th moment $\mu_n$ of a distribution $f(x)$ around a value c is defined as
